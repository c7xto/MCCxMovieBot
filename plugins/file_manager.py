...
async def _show_dupes_page(msg_or_callback, dupes, page=0):
    ...
    fuzzy_count = total - exact_count
    text = (
        f"🔄 **Duplicate Files: {total} groups**\n"
        f"_(🔴 {exact_count} exact  •  🟡 {fuzzy_count} fuzzy — Page {page+1}/{(total+per_page-1)//per_page})_\n\n"
    )
