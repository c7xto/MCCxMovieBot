...
@Client.on_message(filters.command("delete_duplicates") & filters.private & filters.user(ADMIN_ID))
async def delete_duplicates_cmd(client: Client, message: Message):
    """
    /delete_duplicates — scans all clusters and removes duplicate files.
    Keeps the OLDEST copy of each duplicate group (by ObjectId timestamp).
    ...
    try:
        ...
        await status.edit_text(
            ...
            f"✔️ Oldest copy of each file was preserved.\n\n"
            f"_Run /stats to see updated file count._"
        )
    except Exception as e:
        await status.edit_text(f"❌ **Purge failed:** `{e}`")
