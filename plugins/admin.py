"""
updater.py — GitHub auto-updater for MCCxMovieBot

Triggered by:
  • /update  command (admin only in PM)
  • 🔄 Update Bot  button in /admin panel  (callback: upd_start)

Rules:
  • Only .env is NEVER touched
  • Every other file is replaced with the latest from GitHub
  • Bot auto-restarts after update via os.execv
"""
...
async def _do_update(client: Client, status: Message):
    ...
    try:
        await status.edit_text(
            ...
            f"_({protected} protected file skipped: .env)_",
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception:
        pass

    # Step 2 — download every file and write to disk
    updated, failed = [], []
    async with aiohttp.ClientSession() as s:
    ...
    # Step 3 — report result then restart
    lines = [
        "✅ **Update complete!**\n",
        f"📦 Updated : `{len(updated)}` files",
        f"🔒 Skipped : `.env` _(protected — never touched)_",
    ]
    if failed:
    ...
    logger.info("Restarting bot after update via os.execv.")
...
async def cmd_update(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        f"• All files will be replaced with the latest from GitHub.\n"
        f"• `.env` will **never** be touched.\n\n"
        f"⚠️ Bot auto-restarts after update.\n\nProceed?",
        reply_markup=markup,
        parse_mode=ParseMode.MARKDOWN,
        quote=True,
    )
...
# ── Admin panel 🔄 Update Bot button → show confirm prompt ───────────────────
