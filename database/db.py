...
class Database:
    ...
    async def ensure_indexes(self):
        """Creates indexes on all clusters. Idempotent — safe every startup."""
        for i, col in enumerate(self.file_cols):
            try:
                await col.create_index("file_name")
                # Compound index for fast duplicate detection by name+size
                await col.create_index([("file_name", 1), ("file_size", 1)])
                # Index on file_id for fast dedup during bulk indexing
                await col.create_index("file_id", sparse=True)
    ...
    async def save_file(self, media):
        ...
        # Composite duplicate check: file_name + file_size is the true identity.
        # file_id changes every time Telegram re-uploads the same file, so it
        # cannot be used as the sole dedup key.
        for col in self.file_cols:
    ...
    async def save_files_bulk(self, files_list):
        ...
        # Build composite keys: file_name+file_size is the true duplicate identity
        incoming_keys = set()
        for f in files_list:
            incoming_keys.add((f["file_name"], f["file_size"]))

        existing_keys = set()
        for col in self.file_cols:
            # Query only by file_names for efficiency, then filter by size in Python
            ...
            async for doc in cursor:
                existing_keys.add((doc.get("file_name", ""), doc.get("file_size", 0)))

        new_files = [f for f in files_list
                     if (f["file_name"], f["file_size"]) not in existing_keys]
    ...
    async def find_duplicate_files(self):
        """
        Two-pass duplicate scanner using pure MongoDB aggregation.
        Never loads all files into Python RAM.

        Pass 1 — EXACT by file_name + file_size composite:
          The true duplicate identity. Same movie, same size = same file.
          Groups by both fields, keeps first (oldest _id), flags rest for deletion.
          Uses allowDiskUse=True for large collections.

        Pass 2 — EXACT by file_id:
          Catches same Telegram file_id uploaded multiple times.
          Less common but happens with re-forwards.

        Returns list of groups, each with: name, count, ids, keep_id, type
        keep_id = the _id to KEEP (oldest). All other ids in the group = delete.
        """
        results = []

        for col in self.file_cols:
            # Pass 1: group by file_name + file_size (the reference logic)
            try:
                pipeline = [
                    {"$match": {
                        "file_name": {"$exists": True, "$ne": ""},
                        "file_size": {"$exists": True, "$gt": 0}
                    }},
                    {"$group": {
                        "_id": {"file_name": "$file_name", "file_size": "$file_size"},
                        "count": {"$sum": 1},
                        # $min of ObjectId = oldest document (ObjectIds are time-ordered)
                        "keep_id": {"$min": {"$toString": "$_id"}},
                        "all_ids": {"$push": {"$toString": "$_id"}}
                    }},
                    {"$match": {"count": {"$gt": 1}}},
                    {"$sort": {"count": -1}},
                    {"$limit": 500}
                ...
                async for doc in col.aggregate(pipeline, allowDiskUse=True):
                    ...
                    results.append({
                        "name": doc["_id"]["file_name"],
                        "size": doc["_id"]["file_size"],
                        "count": doc["count"],
                        "keep_id": keep_id,
                        "ids": delete_ids,  # IDs to DELETE (not keep)
                        "type": "exact"
                    })
            except Exception as e:
                logger.warning(f"Duplicate scan (pass 1) error: {e}")

            # Pass 2: group by file_id (re-forwarded same file)
            try:
                pipeline2 = [
                ...
                async for doc in col.aggregate(pipeline2, allowDiskUse=True):
                    keep_id = doc["keep_id"]
                    all_ids = doc["all_ids"]
                    delete_ids = [i for i in all_ids if i != keep_id]
                    results.append({
                ...
                logger.warning(f"Duplicate scan (pass 2) error: {e}")

        # Sort: most duplicates first, exact before file_id
        results.sort(key=lambda x: (x["type"] != "exact", -x["count"]))
        results = results[:200]

        # Cache in MongoDB for pagination
        try:
            if self.main_db is not None:
    ...
    async def delete_duplicates_all(self):
        """
        Admin command: scan and auto-delete ALL duplicates across all clusters.
        Keeps the oldest copy of each file (lowest ObjectId = earliest insert).
            ...
            try:
                ...
                async for doc in col.aggregate(pipeline, allowDiskUse=True):
                    ...
                    for oid in doc["all_ids"]:
                        if oid != keep_id:
                            ids_to_delete.append(ObjectId(oid))

                    # Batch delete every 500 to avoid memory spikes
                ...
                logger.warning(f"delete_duplicates_all error on cluster: {e}")
