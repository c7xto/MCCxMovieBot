...
class Database:
    ...
    async def delete_duplicates_all(self):
        """
        Scans all clusters and deletes all duplicate files.
        Keeps the OLDEST copy of each duplicate group (lowest ObjectId = earliest insert).
        Duplicate identity = same file_name + same file_size.
        Runs in batches of 500 to avoid memory spikes.
        Uses allowDiskUse=True so it works on large collections.
        """
        total_deleted = 0
