...
class Database:
    ...
    async def get_db_size(self, db_instance):
        ...
        # Use storageSize — this is what MongoDB Atlas enforces for the 512MB quota.
        # dataSize (compressed docs) was showing ~240MB while Atlas was actually at 512MB.
        # storageSize includes indexes + allocated disk blocks — matches Atlas dashboard.
        storage = stats.get("storageSize", 0)
        data = stats.get("dataSize", 0)
        # Use whichever is larger to be safe
        return max(storage, data) / (1024 * 1024)
