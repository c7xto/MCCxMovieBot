...
class Database:
    ...
    async def find_duplicate_files(self):
        """
        Cursor-based duplicate scanner. Works on Atlas M0 free tier.
        No aggregation — uses indexed sort on (file_name, file_size) instead.
        Consecutive documents with same name+size are duplicates.
        allow_disk_use=True lets MongoDB spill to disk if sort exceeds 32MB RAM.
        """
        results = {}
        ...
        dupes = [v for v in results.values() if v["count"] > 1 and len(v["ids"]) > 0]
    ...
    async def delete_duplicates_all(self):
        """
        Deletes ALL duplicates. Cursor-based — works on Atlas M0.
        Keeps first (oldest by sort) copy of each duplicate group.
        Batches deletions in groups of 500 to avoid memory spikes.
        allow_disk_use=True prevents 32MB sort RAM limit crash.
        """
        total_deleted = 0
