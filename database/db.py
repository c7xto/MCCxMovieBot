...
class Database:
    ...
    async def save_user(self, user_id, first_name):
        ...
        try:
            ...
            return False  # cluster full — user not saved but /start still works
    ...
    async def find_duplicate_files(self):
        """
        Fast duplicate scan using MongoDB $group aggregation — runs entirely
        inside the DB server, never loads files into Python RAM.
        With 1.5M files the old Python loop took minutes; this takes seconds.

        Strategy: run $group on each cluster separately (they are independent
        MongoDB instances so cross-cluster $lookup is not possible). Then merge
        the per-cluster results in Python — but only the duplicate name strings,
        not all 1.5M docs.
        """
        if self.main_db is None:
            return []

        # Step 1: aggregate per cluster — group by file_name, count, collect IDs
        # Only pull names that appear >1 time within that cluster.
        # This is done in the DB server — very fast.
        name_data = {}  # name -> {"count": N, "ids": [...]}
            ...
            try:
                async for doc in col.aggregate(pipeline):
                    ...
                    if name in name_data:
                        name_data[name]["count"] += doc["count"]
                        name_data[name]["ids"].extend(doc["ids"])
                    else:
                        name_data[name] = {
                            "count": doc["count"],
                            "ids": doc["ids"]
                ...
                logger.warning(f"Duplicate scan aggregation error: {e}")
                continue

        # Step 2: filter to only names with total count > 1 across all clusters
        dupes = [
            {"name": name, "count": data["count"], "ids": data["ids"]}
            for name, data in name_data.items()
            if data["count"] > 1
        ]
        dupes.sort(key=lambda x: x["count"], reverse=True)

        # Step 3: cache top 500 in MongoDB for pagination
        dupes_col = self.main_db["duplicate_scan_results"]
        await dupes_col.drop()
        if dupes:
            await dupes_col.insert_many(dupes[:500])
    ...
    async def save_search(self, session_id, data):
        ...
        try:
            await self.cache_col.update_one(
                {"_id": session_id}, {"$set": data}, upsert=True
            )
        except Exception as e:
            # Cluster full or write error — search can still show results
            # from the in-memory results list passed via route_menu
            logger.warning(f"save_search failed (cluster may be full): {e}")
    ...
    async def clear_old_searches(self, expiry_seconds=600):
        ...
        try:
            ...
            pass  # non-fatal — stale sessions just stay until cluster has space
