...
class Database:
    ...
    async def find_duplicate_files(self):
        """
        Duplicate scanner that works on Atlas M0 free tier.
        Does NOT use $group aggregation (not supported with allowDiskUse on M0).

        Strategy: sort by file_name+file_size using a cursor, then compare
        consecutive documents in Python. Only keeps current+previous in memory
        at any time — O(1) memory regardless of collection size.

        Returns top 100 duplicate groups for display in File Manager.
        Each entry: {name, size, count, keep_id, ids (to delete), type}
        """
        results = {}  # key=(file_name, file_size) -> {keep_id, ids, count, name}

        for col in self.file_cols:
            try:
                # Sort by file_name then file_size — consecutive duplicates will be adjacent
                cursor = col.find(
                    {"file_name": {"$exists": True, "$ne": ""},
                     "file_size": {"$exists": True, "$gt": 0}},
                    {"file_name": 1, "file_size": 1, "_id": 1}
                ).sort([("file_name", 1), ("file_size", 1)])

                prev_key = None
                async for doc in cursor:
                    key = (doc.get("file_name", ""), doc.get("file_size", 0))
                    oid = str(doc["_id"])
                    ...
                    if key == prev_key:
                        ...
                        if key in results:
                            results[key]["count"] += 1
                            results[key]["ids"].append(oid)
                        else:
                            # Shouldn't happen but handle gracefully
                            results[key] = {
                                "name": key[0], "size": key[1],
                                "count": 2, "keep_id": oid,
                                "ids": [oid], "type": "exact"
                            }
                    else:
                        if key not in results:
                            # First time seeing this key — store as potential keep_id
                            results[key] = {
                                "name": key[0], "size": key[1],
                                "count": 1, "keep_id": oid,
                                "ids": [], "type": "exact"
                ...
                logger.warning(f"Duplicate scan error on cluster: {e}")

        # Filter to only actual duplicates (count > 1)
        dupes = [
            v for v in results.values()
            if v["count"] > 1 and len(v["ids"]) > 0
        ]
        ...
        try:
            if self.main_db is not None:
                ...
                if dupes:
                    await dupes_col.insert_many(dupes)
        except Exception:
            pass
    ...
    async def delete_duplicates_all(self):
        """
        Deletes ALL duplicates across all clusters without aggregation.
        Works on Atlas M0 free tier. Uses cursor sort + Python comparison.
        Keeps the FIRST (oldest by sort order) copy of each duplicate group.
        Deletes in batches of 500 to avoid memory spikes.
